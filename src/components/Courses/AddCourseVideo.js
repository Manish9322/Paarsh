...
import { useAddCourseVideoMutation, useFetchCourseVideoByIdQuery } from "@/services/api";
...
const AddCourseModal = ({ isOpen, onClose, onAddCourse }) => {
  ...
  const { data: fetchResponse, isLoading: isLoadingCourseVideos, refetch } = useFetchCourseVideoByIdQuery({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (courseVideos && courseVideos.topics && Array.isArray(courseVideos.topics)) {
        ...
        for (let i = 0; i < courseVideos.topics.length; i++) {
          ...
          if (topicIndex < currentState.topics.length) {
            ...
            if (currentState.topics[topicIndex].videos && currentState.topics[topicIndex].videos.length > 0) {
              dispatch(removeVideoFromTopic({
                topicId: topicId,
            ...
            if (topic.videos && Array.isArray(topic.videos)) {
              for (let j = 0; j < topic.videos.length; j++) {
                ...
                const updatedTopic = updatedState.topics.find(t => t._id === topicId);
                
                if (updatedTopic && updatedTopic.videos && updatedTopic.videos.length > 0) {
                  const videoId = updatedTopic.videos[updatedTopic.videos.length - 1]._id;
                  
                  // Update video properties
                  dispatch(updateVideoName({
                    topicId: topicId,
                    videoId: videoId,
                    value: video.videoName
                  }));
                  
                  dispatch(updateVideoId({
                    topicId: topicId,
                    videoId: videoId,
                    value: video.videoId
                  }));
                  
                  console.log(`Added video ${j} with name ${video.videoName} to topic ${i}`);
                }
              }
            }
          }
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-h-[80vh] overflow-y-auto p-6">
          <DialogTitle className="text-2xl font-semibold">
            {courseVideos && courseVideos.topics ? "Update Course Videos" : "Add Course Videos"}
          </DialogTitle>
          ...
          {isLoadingCourseVideos || !dataInitialized ? (
            ...
            <>
              {/* Course Name Display */}
              <div className="mb-4 p-2 bg-gray-100 rounded-md dark:bg-gray-800">
                <p className="font-semibold">Course: {selectedCourse.courseName}</p>
              </div>
              ...
              <div className="">
                ...
                {courseData.topics && courseData.topics.map((topic) => (
                  <div key={topic._id} className="mb-3 mt-2 p-3 border rounded-md">
                    <Input
                      type="text"
                    ...
                    {topic.videos && topic.videos.map((video) => {
                      ...
                      const hasExistingVideo = video.videoId && !video.videoId.startsWith("data:video/") && !videoData;
                      
                      return (
                        <div key={video._id} className="mb-2 flex flex-col gap-2 border-t pt-2">
                          <Input
                            type="text"
                          ...
                          <div className="flex gap-2 items-center">
                            ...
                            {(videoData || hasExistingVideo) && (
                              <Button 
                                type="button"
                                onClick={() => openVideoPreview(videoData ? videoData.preview : video.videoId)}
              ...
              <div className="mt-5 flex justify-end gap-3">
                ...
                <Button
                  ...
                  disabled={isLoading}
                >
                  {isLoading ? "Saving..." : (courseVideos && courseVideos.topics ? "Update Course" : "Save Course")}
                </Button>
              </div>
            </>
          )}
