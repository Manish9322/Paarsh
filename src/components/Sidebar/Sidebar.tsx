...
const Sidebar: React.FC<SidebarProps> = ({ userRole } ) => {
  ...
  return (
    <div className="h-full w-full relative">
      ...
      >
        {/* Sidebar Header */}
        <div className={`flex items-center px-4 py-4 ${isOpen ? "justify-start" : "justify-center"}`}>
        ...
        {/* Sidebar Content */}
        <nav className={`flex-grow overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 ${isOpen ? "pt-2" : "pt-2"}`}>
                  ...
                  <>
                    ...
                    <motion.button
                      ...
                      className={`flex w-full items-center justify-between rounded-lg px-4 py-2.5 text-sm font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-700 ${
                        activeItem === index ? "bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400" : ""
                      }`}
                    >
                      <div className="flex items-center space-x-3">
                        <span className={`flex-shrink-0 ${activeItem === index ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"}`}>
                          {item.icon}
                        </span>
                        {isOpen && <span className="truncate tracking-wide">{item.name}</span>}
                      </div>
                      {isOpen && (
                        <motion.div
                    ...
                    <AnimatePresence>
                      {openSubmenus[index] && isOpen && (
                        <motion.ul
                          ...
                          transition={{ duration: 0.3 }}
                          className="space-y-1 rounded-lg bg-gray-50 pl-4 pr-2 dark:bg-gray-700/50 mx-2 my-1.5 overflow-hidden"
                        >
                          {item.children.map((child, childIndex) => (
                            ...
                            >
                              ...
                              >
                                ...
                                <span className="truncate tracking-wide">{child.name}</span>
                              </Link>
                            </motion.li>
                          ))}
                        </motion.ul>
        ...
        <div className={`border-t border-gray-200 dark:border-gray-700 ${isOpen ? 'p-4' : 'py-4'}`}>
          {isOpen ? (
            <div className="rounded-lg bg-gradient-to-r from-blue-50 to-indigo-50 p-3 dark:from-blue-900/20 dark:to-indigo-900/20 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <motion.button
                  ...
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md text-gray-600 dark:bg-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400"
