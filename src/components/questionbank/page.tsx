...
import { Search, BookOpen, ChevronRight, Clock, Tag, AlertCircle } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            type="text"
            className="bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 shadow-sm"
      ...
      {/* Main Content */}
      {isLoggedIn ? (
        userError || testsError ? (
          // Improved Error View
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="flex flex-col items-center justify-center py-12 px-4 bg-white dark:bg-gray-800 rounded-xl"
          >
            ...
            <p className="text-gray-600 dark:text-gray-400 text-center max-w-md mb-6">
              We encountered an issue while fetching your practice tests. Please try again or contact support if the issue persists.
            </p>
          ...
          >
            {displayedTests.map((test) => (
              <motion.div
                key={test._id}
                variants={item}
                className="bg-white dark:bg-gray-800 rounded-md overflow-hidden shadow-sm hover:shadow-md border border-gray-100 dark:border-gray-700 transition-all duration-300 p-5"
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
