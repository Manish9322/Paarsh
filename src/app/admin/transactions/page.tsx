...
import { useFetchTransactionsQuery, useFetchCourcesQuery, useFetchUsersQuery } from "@/services/api";
...
const TransactionsPage = () => {
    ...
    const { data: transactionsData, isLoading, error: fetchError } = useFetchTransactionsQuery(undefined);
    ...
    const filteredTransactions = transactions.filter((transaction: any) => {
        ...
        if (activeFilters.minAmount && transaction.amount < parseInt(activeFilters.minAmount)) {
            return false;
        }

        if (activeFilters.maxAmount && transaction.amount > parseInt(activeFilters.maxAmount)) {
            return false;
        }
    ...
    return (
        <div className="flex min-h-screen">
            ...
            <div className="fixed left-0 right-0 top-0 z-50 flex h-16 items-center justify-between bg-white px-4 shadow-sm md:hidden">
                ...
                <h1 className="text-lg font-bold text-gray-800">Transaction management</h1>
                <div className="w-10"></div>
            </div>

            {/* Sidebar */}
            <aside
                className={`fixed left-0 top-0 z-40 h-full w-64 transform bg-white shadow-lg transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
                                        ...
                                        <div className="flex items-center">
                                            ...
                                            <div className="ml-4 flex-1">
                                                ...
                                                <p className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
                                                    ₹{transactions.reduce((sum, tx) => sum + (tx.status === "SUCCESS" ? tx.amount : 0), 0).toLocaleString()}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4">
                                        <div className="flex items-center">
                                            ...
                                            <div className="ml-4 flex-1">
                                                ...
                                                <p className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
                                                    {transactions.filter(tx => tx.status === "SUCCESS").length.toLocaleString()}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="mb-6 grid gap-4 md:grid-cols-4">
                                ...
                                <div>
                                    <Input
                                        ...
                                        onChange={(e) => handleFilterChange({ ...activeFilters, minAmount: e.target.value })}
                                        className="w-full"
                                    />
                                </div>
                                <div>
                                    <Input
                                        ...
                                        onChange={(e) => handleFilterChange({ ...activeFilters, maxAmount: e.target.value })}
                                        className="w-full"
                                    />
                                </div>
                                <div>
                                    <Input
                                        ...
                                        onChange={(e) => handleFilterChange({ ...activeFilters, date: e.target.value })}
                                        className="w-full"
                                    />
                                </div>
                            </div>
                                ...
                                <Table className="w-full text-black dark:text-white">
                                    <TableHeader>
                                        <TableRow className="border border-gray-200 bg-gray-50 hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-900 dark:hover:bg-gray-800">
                                            ...
                                            <TableHead className="hidden py-3 md:table-cell">User Name</TableHead>
                                            <TableHead className="hidden py-3 sm:table-cell">Course</TableHead>
                                            <TableHead className="py-3">Amount</TableHead>
                                            <TableHead className="hidden py-3 sm:table-cell">Status</TableHead>
                                            <TableHead className="hidden py-3 md:table-cell">Date</TableHead>
                                            <TableHead className="py-3 text-center">Actions</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {displayedTransactions.length > 0 ? (displayedTransactions.map((transaction: any, index: number) => (
                                            <TableRow
                                                key={transaction._id}
                                    ...
                                    <div className="overflow-hidden rounded-md border border-gray-100 transition-all hover:shadow-md dark:border-gray-700">
                                        ...
                                        <div className="divide-y divide-gray-100 dark:divide-gray-700">
                                            <div className="grid grid-cols-3 px-4 py-3">
                                                ...
                                                <span className="col-span-2 text-sm text-gray-900 dark:text-gray-200">
                                                    {getUserDetails(selectedTransaction.userId?._id)?.name || selectedTransaction.userId?.name}
                                                </span>
                                            </div>
                                            <div className="grid grid-cols-3 px-4 py-3">
                                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                                                ...
                                                <span className="col-span-2 text-sm break-all text-gray-900 dark:text-gray-200">
                                                    {getUserDetails(selectedTransaction.userId?._id)?.email || selectedTransaction.userId?.email}
                                                </span>
                                            </div>
                                            <div className="grid grid-cols-3 px-4 py-3">
                                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                                                ...
                                                <span className="col-span-2 text-sm text-gray-900 dark:text-gray-200">
                                                    {getUserDetails(selectedTransaction.userId?._id)?.mobile || selectedTransaction.userId?.mobile || 'N/A'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="overflow-hidden rounded-md border border-gray-100 transition-all hover:shadow-md dark:border-gray-700">
                                        ...
                                        <div className="divide-y divide-gray-100 dark:divide-gray-700">
                                            ...
                                            <div className="grid grid-cols-3 px-4 py-3">
                                                ...
                                                <span className="col-span-2 text-sm text-gray-900 dark:text-gray-200">
                                                    {getCourseDetails(selectedTransaction.courseId?._id)?.level || 'N/A'}
                                                </span>
                                            </div>
                                            <div className="grid grid-cols-3 px-4 py-3">
                                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                                                ...
                                                <span className="col-span-2 text-sm text-gray-900 dark:text-gray-200">
                                                    {getCourseDetails(selectedTransaction.courseId?._id)?.duration || 'N/A'}
                                                </span>
                                            </div>
                                            <div className="grid grid-cols-3 px-4 py-3">
                                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                                                ...
                                                <span className="col-span-2 text-sm text-gray-900 dark:text-gray-200">
                                                    ₹{selectedTransaction.courseId?.price}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
